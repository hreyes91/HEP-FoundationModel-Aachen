...
def TestAgain(model_dir,model_name_test='best'):
    ...
    test_command='python test_classifier_AL.py --data_path_1 '+data_path_1+' --data_path_2 '+data_path_2+' --model_dir '+ model_dir +'  --num_events '+str(num_events_test)+' --num_const '+str(num_const)+' --pred_name '+'predictions_'+str(model_name_test)+'.npz'+' --model_name '+str(model_name_test)
...
def GetVars(model_dir):
    


    lines=read_file(model_dir+'/arguments.txt')
    
    fine=extract_value('fun_model',lines)

    hlf=extract_value('use_hlf',lines)

    
    schedule=extract_value('scheduler_name',lines)
    
   

    return fine,hlf,schedule
...
for run in top_runs['RUN']:
    ...
    for result in results_dirs:
        ...
        try:
            model_dir=main_result_dir+model_types.get(model_type)+'/'+result
            num_events_train,auc_best,max_sic_best,auc_last,max_sic_last,fpr_best,tpr_best,fpr_last,tpr_last,auc_score_best_train,max_sic_best_train,fpr_best_train,tpr_best_train=TestResults(dict_auc,model_dir,r_tresh)
            PlotTopAUC(fpr_last,tpr_last,run,auc_best,max_sic_best)
...
def PlotCustom(fpr,tpr,df):

    fine = str(df.loc[df['RUN'] == run, 'fine'].values[0])
    hlf = str(df.loc[df['RUN'] == run, 'hlf'].values[0])
    schedule = str(df.loc[df['RUN'] == run, 'schedule'].values[0])
    print(fine)
    colors = {'scratch': 'red', 'freeze': 'blue', 'fine': 'green'}
    linestyles = {'linear': '-', 'False': '--'}
    opacities = {'cos':1, 'reduce': .6}
    
    print('ready to plot')
    plt.plot(tpr,1/fpr,color=colors.get(fine),linestyle=linestyles.get(hlf),alpha=opacities.get(schedule),markersize=1) #color=color)
    plt.ylim(1, 1e5)
    plt.xlim(0, 1)
    plt.xlabel(r"$ tpr$")
    plt.ylabel(r"$1 / fpr$")
...
for run in range(101,103):
    ...
    for result in results_dirs:
        ...
        fpr=rocs.get('fpr_best')[run-101]
    
        PlotCustom(fpr,tpr,all_runs_frame_1000)
...
colors = {'scratch': 'red', 'freeze': 'blue', 'fine': 'green'}
linestyles = {'linear': '-', 'False': '--'}
opacities = {'cos':1, 'reduce': .5}


plot_title='CLS head - supervised (imbalanced) '
handles = []
for fine_value in colors.keys():

    handle = plt.Line2D([0], [0], color=colors[fine_value], label=f"Fine: {fine_value}")
    handles.append(handle)
for hlf_value in linestyles.keys():
        
    handle = plt.Line2D([0], [0],linestyle=linestyles[hlf_value],label=f"HLF: {hlf_value}")
    handles.append(handle)
for schedule_value in opacities.keys():
    handle = plt.Line2D([0], [0], alpha=opacities[schedule_value], label=f"Schedule: {schedule_value}")
    handles.append(handle)
